...
(defn parse-query [{:keys [entity alias args attributes] :as query}
                   {:keys [select from where params vars alias-map] :as parsed}]
  (let [
        new-from (into [] (concat from [[entity alias]]))
        new-alias-map (if (alias-map ) merge alias-map {entity table-alias})
        parsed-args (reduce
                     (fn [curr-parsed arg]
